#!/usr/bin/env python3
"""generate-island-10-audio.py — אודיו לאי 10 (מִפְרַץ אַבְנֵי הַבְּנִיָּה) · C10 + C10b.

קול נוני (ElevenLabs eleven_v3 דרך _eleven_tts):
  ...
  mission-isl10-gender.mp3    — הוראת משחקון 3 (הוּא אוֹ הִיא?)          [C10b]
  mission-isl10-hey.mp3       — הוראת משחקון 4 (הַהַתְחָלָה שֶׁל הַ)       [C10b]
  ...
  C10b · משחקון ה' הידיעה (stem=משפט, options=מילה עם/בלי הַ · two-tap):
  s-isl10-hey-{dag,kelev,yeled,sefer,shemesh}.mp3  — 5 משפטי-stem (נרטיב)
  w-isl10-{hadag,dag,hakelev,kelev,hayeled,yeled,hasefer,sefer,hashemesh,shemesh}.mp3
                                                    — 10 מילות-אופציה (הומופון-QA)

מדוע משחקון הסיומות/זכר-נקבה בלי אודיו פר-מילה? הם עובדים על stem טקסט (הבסיס
נראֶה לעין) → הוראת-המשימה מספיקה. רק stems התחיליות ומשחקון ה' הידיעה מוקלטים.
ב-w-isl10-hadag מול w-isl10-dag: ה' = הברה שלמה → חייבת להישמע (accept={"הדג"}
מול {"דג"}), כך ש-QA התמלול מוכיח שהמסיח מובחן באוזן (כלל המסיחים).
...
NARRATION = {
    ...
    # ─── C10b · הוראות שני משחקוני-ההמשך ───
    "mission-isl10-gender": (
        "לְעֶצֶם זָכָר מַתְאִים תֹּאַר זָכָר, וּלְעֶצֶם נְקֵבָה מַתְאִים תֹּאַר נְקֵבָה. "
        "הִסְתַּכְּלוּ בַּמִּלָּה, וּבַחֲרוּ אֶת הַצּוּרָה הַמַּתְאִימָה."
    ),
    "mission-isl10-hey": (
        "הַ הַקְּטַנָּה בַּהַתְחָלָה אוֹמֶרֶת: זֶה מַשֶּׁהוּ מְיֻחָד שֶׁכְּבָר מַכִּירִים. "
        "הַקְשִׁיבוּ לַמִּשְׁפָּט, וּבַחֲרוּ אֶת הַמִּלָּה שֶׁנֶּאֶמְרָה."
    ),
    # ─── C10b · משפטי-stem למשחקון ה' הידיעה (ה' תמיד נשמעת) ───
    "s-isl10-hey-dag":    "נוּנִי רָאֲתָה אֶת הַדָּג.",
    "s-isl10-hey-kelev":  "הַכֶּלֶב רָץ עַל הַחוֹל.",
    "s-isl10-hey-yeled":  "הַיֶּלֶד קָפַץ לַמַּיִם.",
    "s-isl10-hey-sefer":  "קָרָאתִי אֶת הַסֵּפֶר.",
    "s-isl10-hey-shemesh": "הַשֶּׁמֶשׁ זָרְחָה בַּבֹּקֶר.",
}

# ─── מילות-אודיו — (key, טקסט מנוקד, accept-set של תמלול מנורמל) ───
# תחיליות (C10) + מילות-אופציה עם/בלי ה' הידיעה (C10b). ב-הַX מול X: ה' = הברה
# שלמה שחייבת להישמע → accept-set שונה ("הדג" מול "דג") מוכיח שהמסיח מובחן באוזן.
WORDS = [
    ("isl10-w-lenadav",   "לְנָדָב",    {"לנדב"}),
    ("isl10-w-vechardal", "וְחַרְדָּל",  {"וחרדל"}),   # ו'-החיבור חייבת להישמע
    ("isl10-w-bidvash",   "בִּדְבַשׁ",   {"בדבש"}),
    # C10b · זוגות ה' הידיעה (הַX חייב להישמע שונה מ-X)
    ("isl10-w-hadag",     "הַדָּג",     {"הדג"}),
    ("isl10-w-dag",       "דָּג",       {"דג"}),
    ("isl10-w-hakelev",   "הַכֶּלֶב",    {"הכלב"}),
    ("isl10-w-kelev",     "כֶּלֶב",      {"כלב"}),
    ("isl10-w-hayeled",   "הַיֶּלֶד",    {"הילד"}),
    ("isl10-w-yeled",     "יֶלֶד",       {"ילד"}),
    ("isl10-w-hasefer",   "הַסֵּפֶר",    {"הספר"}),
    ("isl10-w-sefer",     "סֵפֶר",       {"ספר"}),
    ("isl10-w-hashemesh", "הַשֶּׁמֶשׁ",  {"השמש"}),
    ("isl10-w-shemesh",   "שֶׁמֶשׁ",     {"שמש"}),
]
