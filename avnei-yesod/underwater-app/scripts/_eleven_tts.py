...
KNOWN_WORD_FIXES = {
    "תשובה": "תשובה",
    # ספר: אי 2 קיבל רפה (8.7); אי 14 (מיטל 9.7) — בלי רפה נהגה נכון /sefer/. הרפה
    # דווקא קלקלה. הערך אחיד: סֵפֶר. (קליפי אי 2 הקיימים לא מושפעים עד רה-יצירה.)
    "ספר":  "סֵפֶר",
    # מפתח: הניקוד המלא (פתח-גנובה תחת ח') נהגה "מפתחה". בלי תנועה על ח'.
    "מפתח": "מַפְתֵּח",
    # סבא: הדגש ב-ב' לא כובד (נשמע "סוא"). כתיב-חסר נהגה /saba/ תקין.
    "סבא":  "סבא",
    # אי 14 (מיטל 8.7): "בנו/built" נהגה "בָּאנוּ/came" (א' טפילה). דגש בנ' מחבר
    # את הצליל ומונע את העצירה. אושר באוזן (banu-compare-2/3).
    "בנו":  "בָּנּוּ",
    # גבוה: פתח גנוב על ה' סופית מוסיף הברה ("גבוהֶה"). כתיב-חסר נהגה נכון.
    "גבוה": "גבוה",
    # שניהם: הכתיב המנוקד לעיתים נשבר; כתיב-חסר יציב ותקין (אושר באוזן).
    "שניהם": "שניהם",
    # אי 14 (מיטל 9.7): כתיב-חסר יציב יותר מהמנוקד (אושר באוזן).
    "יפה":  "יפה",     # יָפֶה — הפ' הרפה נשברה במנוקד
    # הכינה: eleven נתן דגש בכ' (/hekina/); רפה על הכ' (ֿ) כופה כ' רכה /hexina/.
    "הכינה": "הֵכִֿינָה",  # אושר באוזן 9.7 (hkr-2, המשפט המלא)
    "משחק": "משחק",    # מִשְׂחָק — כתיב-חסר תקין (אושר באוזן 9.7)
    # הברה: ה-ב' הרפה נשמעה /b/ במקום /v/ ← סימן רפה (ֿ). אושר באוזן.
    "הברה": "הֲבָֿרָה",
    # אי 14 (מיטל 9.7): עוד כתיב-חסר (המפתח = שלד הקטע המנוקד; לרוב חסר-יו').
    "דוכן":  "דוכן",       # דּוּכַן
    "התישבה": "התיישבה",   # הִתְיַשְּׁבָה → מלא (2 יו') נהגה נכון
    "פרחים": "פרחים",      # פְּרָחִים
    "וכוח":  "ויכוח",      # וִכּוּחַ → מלא (עם יו') נהגה נכון
    "כתבה":  "כתבה",       # כָּתְבָה
    # מיפוי (מיטל 20.7): "שְׁמַעְתֶּם" — השווא-הנע ב-ש' נבלע ונשמע "שמעתם" חטוף.
    # מיטל ביקשה פתח מפורש ב-ש' → צליל /a/ ברור לילדי א'. חל גם על "שֶׁשְּׁמַעְתֶּם".
    "שמעתם":  "שַׁמַעְתֶּם",
    "ששמעתם": "שֶׁשַּׁמַעְתֶּם",
}
