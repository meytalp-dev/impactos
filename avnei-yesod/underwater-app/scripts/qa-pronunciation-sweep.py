...
MANIFEST = ADMIN / "audio-manifest.json"              # ליבה פונטית (build-audio-manifest.js)
MANIFEST_WORDS = ADMIN / "audio-manifest-words.json"  # מילים+נרטיב (build-audio-manifest-words.py)
...
def load_manifest():
    """ממזג את יעד-הליבה הפונטי + יעד המילים/נרטיב (הליבה גוברת בהתנגשות)."""
    m = {}
    if MANIFEST_WORDS.exists():
        m.update(json.loads(MANIFEST_WORDS.read_text(encoding="utf-8")))
    if MANIFEST.exists():
        m.update(json.loads(MANIFEST.read_text(encoding="utf-8")))  # ליבה אחרונה = גוברת
    return m


def score(rec, manifest):
    """מחשב intended/sim/suspect לרשומה יחידה מתוך ה-heard הקיים."""
    heard = rec.get("heard", "")
    intended = manifest.get(rec.get("_key", ""))
    if intended:
        rec["intended"] = intended
        rec["sim"] = round(similarity(intended, heard), 2)
        rec["suspect"] = rec["sim"] < 0.6 or not strip_niqqud(heard)
    else:
        rec.pop("intended", None)
        rec.pop("sim", None)
        rec["suspect"] = not strip_niqqud(heard)
    return rec


def recompare(manifest):
    """סבב-השוואה בלבד — בלי תמלול-מחדש. משתמש ב-heard הקיים ומחשב sim מחדש
    מול ה-manifest המורחב (מוסיף יעד למילים/נרטיב שקודם היו בלי)."""
    results = load_results()
    if not results:
        print("אין תוצאות קיימות להשוואה — הרץ קודם את הסריקה המלאה.", flush=True)
        return
    before = sum(1 for r in results.values() if "intended" in r)
    for key, rec in results.items():
        rec["_key"] = key
        score(rec, manifest)
        rec.pop("_key", None)
    after = sum(1 for r in results.values() if "intended" in r)
    save(results, MODEL_SIZE, len(results), len(results))
    suspects = sum(1 for r in results.values() if r.get("suspect"))
    print(f"✓ סבב-השוואה: יעד לפני {before} → אחרי {after} · {suspects} חשודים · {len(results)} קליפים", flush=True)
...
def main():
    ...
    if "--recompare" in args:   # סבב-השוואה בלבד, בלי תמלול (מהיר, בלי Whisper)
        recompare(manifest)
        return
