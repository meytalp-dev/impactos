...
1. **אודיו = Web Speech he-IL בלבד.** AvriNeural MP3 = post-pilot.
   - **דלג על Phase C.8** — אל תיצור `tools/generate-island-14-audio.js`.
   - **Phase C.7** הופך ל: ב-html, קריאה ישירה ל-`SpeechSynthesisUtterance(text, {lang: 'he-IL'})`, ללא MP3 lookup.
   - **Phase C.9** (Auto-fallback) — לא רלוונטי, אין MP3 לחפש.
   - חוסך ~2-3 שעות מהאומדן (Phase C היה 3h, עכשיו ~30 דק').
2. **Passages חסרים** — לא להמציא. לבקש ממיטל או להשתמש ב-task_3 הקיים ב-`moy-items.json` כתבנית (לא לערוך את הקובץ — רק לקרוא ולשאוב מבנה).
3. **Sub-BKT** — לא הוחלט מפורשות. **לעצור ולשאול את מיטל לפני שינוי ב-`bkt.js`** (אם בכלל צריך — הבנת הנשמע יכולה להיות אחוז-הצלחה פשוט, לא BKT מלא).
