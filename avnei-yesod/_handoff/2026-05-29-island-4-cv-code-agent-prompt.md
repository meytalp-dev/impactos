...
1. **Sub-BKT level = פר אות.** לא פר CV pair · לא פר vowel.
   - מבטל את ההמלצה ב-Phase A.4 ("משולב — פר CV pair").
   - מבטל את ה"בספק" בסעיף "בספק" למטה (sub-BKT decision).
   - בפועל: 22 BKT states (אות אחת), לא 154 (22×7) ולא 7 (vowel).
2. **אודיו = Web Speech he-IL בלבד** (כמו fallback). לא להפיק MP3 ל-CV pairs בשלב זה. AvriNeural = post-pilot.
3. **Vocab חסר** — להמשיך לבקש ממיטל לפני המצאה (זה כבר ב-bootstrap, נשאר).
