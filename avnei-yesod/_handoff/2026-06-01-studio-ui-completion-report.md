...
> שלום מיטל. סיימתי את כל 5 השלבים (A→E) של ה-bootstrap. הסטודיו נטען ב-`teacher-studio.html?guest=1` עם wizard שלם, preview iframe חי, validator עברי, feedback widget ו-tour. 7/7 בדיקות Playwright עוברות + 6 צילומי מסך לסקירה.

## TL;DR

- ✅ **7 קבצים חדשים** ב-`avnei-yesod/underwater-app/` (לפי הסקופ של ה-bootstrap + `_shared.js` משותף, כפי שמוּתר).
- ✅ **2 קבצי בדיקות** ב-`avnei-yesod/scripts/e2e/` (smoke + screenshots).
- ✅ **0 קבצים מחוץ לסקופ** נערכו. אפס conflict עם סוכן אי 5 / cloud.
- ✅ **0 שגיאות קונסול** ב-`?guest=1`.
- ⚠️ דורש קצת תיאום עם Pipeline agent (פרטים למטה).
...
## תיאום עם Pipeline Agent — TODO

ה-API שהשתמשתי בו תואם **בדיוק** ל-bootstrap. הנה ה-shape ש-Pipeline agent חייב לספק:
...
**שינויים שצריכים הסכמה:**
1. ב-`_pipeline-stub.js` הוספתי `__stub: true` flag. כש-Pipeline agent טוען את `_pipeline-real.js` חייב להגיע **אחרי** ה-stub ולעשות `window.AvneiStudioPipeline = realImpl` (לא merge). או לסמן `__stub: false` כדי שה-stub יוותר.
2. **Error codes** שמופו ל-Hebrew ב-`validator-ui.js`:
   - `missing_letter`, `missing_mechanic`, `missing_island`, `missing_audio`
   - `distractors_too_few`, `distractors_too_similar`
   - `niqud_violation_bgd_kpt`, `niqud_missing`
   - `pairs_too_few`, `groups_too_few`, `too_few_items_in_pack`
   - **אם Pipeline מחזיר code אחר** → fallback ל-`err.hebrew` או "משהו דורש תשומת לב" generic. אם Pipeline רוצה להוסיף code חדש — שיוסיף ל-`ERROR_HEBREW` ב-`validator-ui.js` (אני הבעלים של הקובץ).
3. **Audio button:** הקוד שלי מציג כפתור "🟡 נקדי לי" → קורא ל-`applyNiqud()`. כפתור TTS (`generateTTS`) טרם חיברתי לטופס — מומלץ ש-Pipeline agent ייצור אודיו ברקע כשמשתמשת לוחצת "פרסמי" ולא בלחיצה ידנית (חיסכון בקריאות API).
...
- Files changed: 10 new, 0 modified outside scope
- Tests: 7/7 pass (chrome desktop) + 6 screenshots saved
- Blockers: 0 — UI works on _pipeline-stub.js while Pipeline agent finishes
- Next: Pipeline agent replaces window.AvneiStudioPipeline; verify error code map
```

**ממליצה:**
- לסגור את ה-handoff הזה עם commit מקומי `studio-ui: phase A→E complete (7/7 tests, 6 screenshots)` — אני **לא** דוחפת.
- אחרי ש-Pipeline agent מסיים, להריץ שוב את `13-studio-ui.spec.js` ולוודא שהבדיקות עוברות עם ה-real API (יכול להיות שצריך לעדכן את ה-spec).
- לבקש מ-לירון / אופיר / עמית להריץ flow מקצה לקצה לפני 1.6 ולמלא את ה-feedback widget.
