...
## ✅ צ'קליסט מסירה
- [x] **6 × orbs/orb-{red,blue,yellow,green,orange,purple}.png — סופקו ע"י מיטל (13/7), עברו rembg (שקיפות), הוקטנו ל~320px, ושובצו במשחקון.** צורה זהה, רק גוון משתנה. ✓
- [ ] 4 × orbs/orb-{pink,black,white,brown}.png (גל שני — עדיין חסר)
- [ ] (אופ') flower/*.png · ball/*.png לרב־ייצוג
- [x] אימות: 3 גוונים זה־ליד־זה — הצורה **זהה לחלוטין**, רק הצבע שונה (אומת בצילום headless). ✓

---

## מצב נוכחי (2026-07-13, עודכן ערב)
**6 צבעי־הליבה חיים עם נכסים אמיתיים.** מיטל סיפקה 6 כדורי־אור זוהרים (אותו כדור, גוון משתנה, עם סמל־זוהר פנימי). עברו `rembg` → שקיפות אמיתית → הוקטנו ל~320px → `assets/colors/orbs/orb-*.png`. שובצו ל־`color-beam.html` **בלי לגעת במכניקה המשותפת** (`light-beam.js`): המשחקון מעביר את ה־PNG כ־`<img>` דרך מפת ה־glyph המקומית שהמכניקה מזריקה. במצב־מנוחה הכדור מעומעם (grayscale) — האלומה "צובעת" אותו לגוון מלא. אומת בצילום headless (מצב מעומעם + מצב חשוף).

**נותר:** גל שני (pink/black/white/brown) — האודיו והבנק כבר קיימים; חסרים 4 כדורי־אור בגוונים האלה (אותה הפקה, אותם 4 גוונים) כדי להכניסם למשחקון. עד אז המשחקון מריץ 6 ליבה.
