...
export function ProgressPath({ steps, currentIndex, onStepSelect, className = '', ...props }: ProgressPathProps) {
        ...
        {steps.map((step, index) => {
          ...
          const station = (
            <Station
              ...
              index={index + 1}
              state={state}
              asButton={Boolean(onStepSelect)}
              onClick={onStepSelect ? () => onStepSelect(index, step) : undefined}
